...
#ifndef WORLD_MODELS_VISUALIZATION_HPP
#define WORLD_MODELS_VISUALIZATION_HPP
...
public:
    ...
    void updateVelocityVisual(raisim::ArticulatedSystem *&robot,
                              Eigen::Matrix<double, 3, 1> &desiredVelocity,
                              const std::unique_ptr<raisim::RaisimServer> &server) {
        if (!visualizable_) return;

        baseRPY_.e() = rotationMatrixToRPY(robot->getBaseOrientation().e());

        /// Heading velocity visual
        baseRPY_[0] = 0.;
        baseRPY_[1] = M_PI_2;
        baseRotationHeadingComponent_.e() = rpyToRotationMatrix(baseRPY_.e());
        ...
        headingVisualOffset_ = robot->getBaseOrientation() *
                               raisim::Vec<3>{desiredVelocity[0] * 0.25, 0., 0.};
        ...
        server->getVisualObject("heading_velocity")->setCylinderSize(
                0.02, softSign(std::abs(desiredVelocity[0])) * 1.5);
        ...
        lateralVisualOffset_ = robot->getBaseOrientation() *
                               raisim::Vec<3>{0., desiredVelocity[1] * 0.25, 0.};
        robot->getBasePosition(basePosition_);
        ...
        server->getVisualObject("lateral_velocity")->setCylinderSize(
                0.02, softSign(std::abs(desiredVelocity[1])) * 1.5);
        ...
        server->getVisualObject("yaw_rate")->setCylinderSize(
                0.2 * softSign(std::abs(desiredVelocity[2])), 0.05);

        if (desiredVelocity[2] < 0) {
...
#endif //WORLD_MODELS_VISUALIZATION_HPP
