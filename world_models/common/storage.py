...
class ReplayBuffer:
    ...
    def __len__(self):
        return (
            sum([size(eps) for eps in self.episodes])
            + size(self.current_episode) * self.num_envs
        )
    ...
    def store(self, step):
        ...
        self.current_episode = {
            k: torch.cat([self.current_episode[k], v.unsqueeze(0)], dim=0)
    ...
    def store_all_from_tensors(self, data):
        data = {
            k: [x.squeeze(1) for x in torch.split(v, 1, dim=1)] for k, v in data.items()
        }
        ...
        if size(self.current_episode) >= self.chunk_length:
            for env in range(self.num_envs):
                self.episodes.append(
                    {k: v[:, env] for k, v in self.current_episode.items()}
                )
        self.clear_current_episode()

    def clear_current_episode(self):
        self.current_episode = {
            k: torch.empty((0, self.num_envs, v)).to(self.device)
    ...
    def sample(self, batch, chunk_length=None):
        ...
        start = np.random.randint(
            [size(ep) - chunk_length + 1 for ep in eps], size=batch
        )
        end = start + chunk_length
        samples = {
            k: [ep[k][s:e] for ep, s, e in zip(eps, start, end)]
    ...
    def sample_all(self, chunk_length):
        samples = {
            k: torch.split(self.episodes[0][k], chunk_length, dim=0)
            for k in self.dims.keys()
        }
        return {k: torch.stack(v).swapaxes(0, 1) for k, v in samples.items()}
...
class ExpertSampler:
    ...
    def __init__(self, config, data):
        assert (
            size(data) % config.batch_length == 0
    ...
    def __next__(self):
        ...
        samples = {
            k: v[
                self.idx * self.batch_length : (self.idx + 1) * self.batch_length,
    ...
    def sample(self, batch_length, batch_size):
        ...
        return {
            k: v[s:e, idx] for s, e in zip(start, end) for k, v in self.data.items()
        }
