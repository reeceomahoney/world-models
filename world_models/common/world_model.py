...
class WorldModel(nn.Module):
    def __init__(self, obs_dim, act_dim, config):
        ...
        if config.z_dist == "Categorical":
            ...
            self._encoder = models.MultivariateGaussianMLP(self.h_dim + obs_dim, config)
            self._dynamics = models.MultivariateGaussianMLP(self.h_dim, config)
        else:
            raise NotImplementedError("Unknown z_dist")

        # prediction
        self._decoder = models.Decoder(
            self.h_dim + self.z_dim, obs_dim, layers, act, device
        )
        self._reward_model = models.TwoHotSymlogMLP(config)
        self._cont_model = models.BernoulliMLP(
            self.h_dim + self.z_dim, 1, layers, act, device
        )
