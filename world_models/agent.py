...
class Agent(torch.nn.Module):
    ...
    def train_world_model(self, expert_sampler, train=True):
        ...
        dyn_loss = (
            self.config.beta_dyn
            * self.kl_div(d(states["post"].detach()), d(states["prior"])).mean()
        )
        repr_loss = (
            self.config.beta_repr
            * self.kl_div(d(states["post"]), d(states["prior"].detach())).mean()
        )
        ...
        if train:
            ...
            torch.nn.utils.clip_grad_norm_(
                self.world_model.parameters(), self.config.model_grad_clip
            )
        ...
        self.logger.log(
            "world_model",
            {"pred_loss": pred_loss.item(), "kl_loss": (dyn_loss + repr_loss).item()},
        )
    ...
    def encode_data(self, data, h_init=None):
        ...
        for t in range(batch_length):
            ...
            if self.config.z_dist == "Categorical":
                ...
                (post, post_stats), (prior, prior_stats) = self.world_model.get_z_dists(
                    h_t, obs
                )
                z_t = post.sample()
                latents = {
    ...
    def _train_ensemble(self, data, states):
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(
            self.ensemble.parameters(), self.config.model_grad_clip
        )
    ...
    def _train_actor_critic(self, states_0):
        ...
        for _ in range(self.config.imag_horizon):
            ...
            info["size"] += act_t.square().mean() / self.config.imag_horizon
            info["std"] += actor.stddev.mean() / self.config.imag_horizon
        ...
        self.action_log_probs = torch.stack(action_log_probs, dim=0).unsqueeze(-1)
        if self.config.z_dist == "Categorical":
            self.states_logits = torch.stack(states_logits, dim=0)

        # mean distance between agent and expert actions
        act_diff = torch.norm(self.actions - self.expert_actions[1:], dim=-1).mean()
        ...
        self.logger.log(
            "actor_critic_loss", {"policy_loss": policy_loss, "value_loss": value_loss}
        )
    ...
    def _calculate_policy_loss(self):
        ...
        for t in reversed(range(self.config.imag_horizon - 1)):
            if t == self.config.imag_horizon - 2:
                ...
                target = (1 - self.config.lam) * values[
                    t + 1
    ...
    def _calculate_ditto_rewards(self):
        ...
        reward /= (
            torch.maximum(torch.norm(expert_states, dim=-1), torch.norm(states, dim=-1))
            ** 2
        )
    ...
    def _calculate_gammas(self):
        ...
        if self.config.ditto:
            ...
            self.gammas = self.config.gamma * self.world_model.cont(self.states)
            self.weights = torch.cumprod(self.gammas, dim=0).detach()

    def set_expert_data_size(self, expert_sampler):
        self.expert_data_size = (expert_sampler.len_samples, expert_sampler.n_batches)
