 ...
 public:
  ...
  void initializeFromConfigurationFile(const Yaml::Node& cfg) {
    for(auto rw = cfg.Begin(); rw != cfg.End(); rw++) {
      ...
      RSFATAL_IF((*rw).second.IsNone(), "Node " + (*rw).first + " or its coefficient doesn't exist");
      rewards_[(*rw).first].coefficient = (*rw).second.template As<double>();
    }
  }
  ...
  void record (const std::string& name, double reward, bool accumulate = false) {
